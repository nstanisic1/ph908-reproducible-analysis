# config_analysis.R
# Global configuration for the reproducible PH908 analysis pipeline
...
resolve_project_dir <- function() {
  env_root <- Sys.getenv("PH908_PROJECT_DIR", unset = "")
  if (nzchar(env_root)) {
...
if (!dir.exists(PROJECT_DIR)) {
  stop(
    paste0(
      ...
      PROJECT_DIR,
      "\n\nSet PH908_PROJECT_DIR or place config_analysis.R in the project directory."
    ),
    call. = FALSE
  )
}

DIR_ROOT <- normalizePath(PROJECT_DIR, winslash = "/", mustWork = TRUE)
...
# -----------------------------
# Preprocessing QC / support outputs
# -----------------------------
