# 01_preprocessing.R
# Deterministic preprocessing for the PH908 comparative analysis.
...
rel_path <- function(path, root = DIR_ROOT) {
  ...
  out <- sub(paste0("^", gsub("([][{}()+*^$|\\?.])", "\\\\\\1", prefix)), "", path_norm)
  if (identical(out, path_norm) && identical(path_norm, root_norm)) "." else out
}
...
mojibake_rate <- function(x) {
  ...
  mean(stringr::str_detect(x, "Ð|Ñ|Ã"))
}
...
write_facts_header(
  ...
  script_name = "01_preprocessing.R",
  input_files = c(
    FILE_DNK_RECON_CSV,
    FILE_PH908_MAP_TSV,
    FILE_PH908_NODE_AGE_TIERS_TSV,
...
flow_df <- tibble::tibble(
  ...
  step_label = c(
    ...
    "Surname–slava–location clusters",
...
table_preprocessing_supp <- tibble::tibble(
  metric = c(
    ...
    "Unique Location × Region pairs checked",
    "Reviewed cache rows",
...
append_facts_lines(FILE_FACTS_PREPROCESSING, c(
  ...
  paste0(
    ...
    geocode_summary$unique_location_region_pairs_checked,
    " unique Location × Region pairs were checked against the reviewed geocoding cache, and no required pairs were absent."
  ),
  paste0(
